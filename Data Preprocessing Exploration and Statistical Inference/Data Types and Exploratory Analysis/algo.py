...
def find_input(name: str) -> Path:
    """Locate an input file: runtime dataset mount → image-baked dir → local."""
    for base in (INPUTS, Path("/app/data"), _HERE):
        candidate = base / name
        if candidate.exists():
            return candidate
    raise FileNotFoundError(f"{name} not found in {INPUTS}, /app/data or {_HERE}")
...
DATASET_PATH = find_input("corporate_financial_data.csv")
PLOTS_DIR    = OUTPUTS
