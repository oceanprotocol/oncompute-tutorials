...
# ── Paths (Ocean C2D aware) ───────────────────────────────────
# On an Ocean node the container gets a /data volume:
#   inputs  → /data/inputs   (dataset files, or baked into the image)
#   outputs → /data/outputs  (node tars this dir and returns it)
# Override with the INPUTS / OUTPUTS env vars. Falls back to the
# script directory so the script still runs locally unchanged.
_HERE   = Path(__file__).parent
INPUTS  = Path(os.environ.get("INPUTS", "/data/inputs"))
OUTPUTS = Path(os.environ.get("OUTPUTS", "/data/outputs"))


def find_input(name: str) -> Path:
    """Locate an input file: runtime dataset mount → persistent-storage bucket
    → image-baked dir → local."""
    bases = [INPUTS, Path("/app/data"), _HERE]
    # Ocean persistent-storage mounts land at /data/persistentStorage/<bucketId>/<file>.
    ps_root = Path("/data/persistentStorage")
    if ps_root.exists():
        bases += sorted(p for p in ps_root.iterdir() if p.is_dir())
    for base in bases:
        candidate = base / name
        if candidate.exists():
            return candidate
    raise FileNotFoundError(
        f"{name} not found in {[str(b) for b in bases]}"
    )
...
DATA_DIR  = _HERE          # kept for backward compatibility
PLOTS_DIR = OUTPUTS
...
personal = pd.read_csv(find_input("employees_personal_messy.csv"), dtype=str)
work     = pd.read_csv(find_input("employees_work_messy.csv"),     dtype=str)
perf     = pd.read_csv(find_input("employees_performance_messy.csv"), dtype=str)
...
# ── Save ──────────────────────────────────────────────────
out_path = OUTPUTS / "employees_recovered.csv"
...
extended = pd.read_csv(find_input("employees_extended.csv"))
